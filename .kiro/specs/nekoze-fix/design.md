...
- 電源ボタンによるユーザーロックの防止、OS 側の非アクティブ時強制ロック設定への介入（監視中の自動スリープ抑止のみ担当）
...
- wake lock の所有権を「監視中」からアクティブ中常時、あるいは暗転モードへ移す場合（現行は phase == .monitoring 不変条件。ADR 0016）
...
| Q2 | 暗転輝度 | 0.0（全黒）。（2026-09-14 改訂: wake lock は監視中のみ ON の phase 不変条件へ。暗転 enter/exit は輝度のみ扱い、監視中の暗転では点灯が継続する。ADR 0016） |
...
| Q14 | 回転完了判定 | 5秒タイムアウト |
| Q15 | wake lock の実装 | `UIApplication.shared.isIdleTimerDisabled = (snapshot.phase == .monitoring)`。（2026-09-14 改訂: 所有権をアクティブ中常時のライフサイクルから「監視中」限定へ変更。phase 遷移の唯一の書き込み経路 `setPhase` で不変条件を強制。暗転 enter/exit は輝度のみ。監視停止・校正中・idle・権限画面では OFF。要件 8.3/8.4 改訂対応、ADR 0016） |
...
| PostureSessionManager | Session | セッション状態の唯一の所有者。監視中の wake lock 所持（phase 不変条件） | 2.*, 3.*, 4.*, 5.*, 6.*, 7.*, 8.* | 上記すべて | State, Service |
...
- **wake lock（2026-09-14 改訂・8.3/8.4）**: `isIdleTimerDisabled = (snapshot.phase == .monitoring)` を phase 遷移の唯一の書き込み経路 `setPhase()` で強制する不変条件に変更（監視中のみ ON。ADR 0016）。`bootstrap()` は権限分岐で `setPhase(.calibrating)` 等へ遷移するため、監視へ入るまで OFF。暗転 enter/exit は輝度のみ扱い、監視中の暗転では phase が .monitoring のままなので点灯が継続する（8.4: 暗転解除操作と無関係）。`.inactive` では phase が変わらず wake lock も不変
- **背面移行**: 通知音停止、カメラ停止。暗転フラグ・輝度はこの時点では変更しない。監視中/校正中/回転中は `setPhase(.idle)` で退避し、この遷移が wake lock を OFF にする（監視中からの退避で OS 標準へ復元。要求 8.3）。監視フラグ（SettingsStore）は維持 — ユーザーストップではないため
...
- **表示用 dwell ゲート（`postureDisplayGate`, 0.5秒）**: `displayedPosture` の `.slouch` 反映にだけ掛かるもう一段のゲート。`slouchCandidate` が **0.5秒連続**した時点で `.slouch` を表示し、`.good` は即時反映する（フレームノイズによる一瞬の猫背誤表示を防ぐ。通知音を決める 3秒の `slouchGate` とは独立）。**姿勢判定不能フレーム**（`presence == .personDetected` だが `sample == nil`:肩欠測猶予中・キーポイント不足）では表示を維持しつつ、このゲートの蓄積だけ解除する — 欠測を挟んだ非連続フレームで `.slouch` が確定するのを防ぐ。監視開始および**再校正完了**時に初期化する（`resetMonitoringState()` が唯一の初期化点。ゲート蓄積・`lastGateTickTime`・`displayedPosture` をまとめてリセットし、校正中の経過時間が `deltaTime` に乗るのを防ぐ）
...
- **MonitorView**: カメラプレビューは表示しない。校正で確定した姿勢を薄いグレー（`.reference` モード）で固定表示し、現在の姿勢をカラー（`.current` モード）で重ねて表示。開始停止、閾値（角度スライダーと距離スライダーを横並び。FQ3）、暗転ボタン。暗転時は **完全黒画面＋輝度 0.0**（Q2 決定。wake lock は phase == .monitoring の不変条件が担うため暗転では扱わない。監視中の暗転で点灯は継続）。タップで復帰（3.*, 4.4, 6.*）
...
- `enterDimMode()` で:
  ...
  3. ~~`isIdleTimerDisabled = true`~~（廃止：wake lock は phase == .monitoring の不変条件が担う。暗転では触らない。ADR 0016）
- `exitDimMode()` で:
  1. ~~`UIApplication.shared.isIdleTimerDisabled = false`~~（同上、削除。監視中の暗転解除では phase が不変のため点灯が維持される。8.4）
  2. 保存した輝度値へ復元
- バックグラウンド移行時の扱い（タスク 3.5 実装済み）:
  1. `setPhase(.idle)` 経由で `isIdleTimerDisabled = false`（phase 不変条件による OFF。監視中からの退避のみが点灯を終了する）
...
sequenceDiagram
    ...
    Note over Session: 監視中（phase == .monitoring）は wake lock ON（8.3/8.4）
    User->>Monitor: 暗転ボタンタップ
    Monitor->>Session: enterDimMode
    Session->>App: brightness = 0.0
    Note over Session: 元の輝度値をインスタンス変数に保存。phase は不変なので点灯継続
    User->>Monitor: 画面タップ
    Monitor->>Session: exitDimMode
    Session->>App: brightness = saved value
    Note over Session: wake lock は暗転の解除操作と無関係に維持（8.4）
```

### 監視中の自動スリープ抑止（8.3/8.4）
...
sequenceDiagram
    ...
    participant Session as PostureSessionManager
    participant App as UIApplication
    User->>Session: startMonitoring / 校正完了
    Session->>Session: setPhase(.monitoring)
    Session->>App: isIdleTimerDisabled = true
    Note over Session: 監視中のみ点灯。校正中・idle・権限画面は OFF のまま
    User->>Session: stopMonitoring / 監視中に背面移行（setPhase(.idle)）
    Session->>App: isIdleTimerDisabled = false
    Note over Session: phase が監視中を離れた瞬間に OS 標準へ復元（8.3）
```

書き込みは `setPhase()` の1経路のみ。phase が `.monitoring` のときだけ ON なので、退避・明示停止・校正やり直し・権限画面への遷移がすべて同じ式で OFF になる。`.inactive` では phase が変わらず wake lock も不変。
...
- 暗転中のタップは exitDimMode を呼ぶ、輝度復元のみ（監視中なら phase 不変で wake lock 維持）
- 監視開始（startMonitoring / 校正完了）で isIdleTimerDisabled = true、監視停止・背面退避（setPhase(.idle)）で false（8.3 改訂）
...
- 画面暗転時はプレビューレイヤを外し、**輝度 0.0** で消費を抑える（wake lock は phase == .monitoring の不変条件で暗転とは無関係）
- AlertPlayer は **監視開始時にプリロード**（Q17 決定）
- 角度計算は **移動平均なし**（Q12 決定）：生の角度を 3秒ゲートに投入
- UI 更新は判定結果の変化時と表示用間引き（最大 15 Hz）に限定する
- **ランドスケープ時のオーバーレイ座標ずれ（受容リスク）**: プレビューの AspectFit 変換と Vision 正規化座標の対応が非正方形フレームで一致せず、ランドスケープ時にキーポイント描画がプレビューからずれ得る。判定ロジック（角度・距離）は正規化座標で完結するため影響しない。PR #9 で補正方向の修正（commit 92cfd61）と `videoOrientation` の受け渡し（commit 8bb55ce）まで実施したが、ランドスケープのアスペクト比補正はポートレート時のデグレを招くため保留（8bb55ce「別途調査後に追加予定」）。表示のみの既知の制約として受容する
- **自動スリープ抑止の消費（8.3/8.4 改訂）**: 抑止は監視中に限定されるため、idle 滞留・校正中の画面オフは OS 標準どおり発生してよい（点灯常時前提の旧再実測は失効、ADR 0016）。NFR 9.1 は監視継続＝点灯前提で実機確認。iPad の Split View/Slide Over で本アプリシーンが非フォーカスでも点灯が遅延する場合はあるが、OS 側調停への介入はスコープ外（Adjacent Expectations 準拠、受容リスク）
...
| 6.1 | 暗転へ切替（輝度 0.0。wake lock は 8.3 で監視中の不変条件） | MonitorView, PostureSessionManager | enterDimMode | 暗転 |
...
| 8.3 | 監視中は自動スリープで画面オフにならない／退避・監視停止で OS 標準へ復元 | PostureSessionManager, RootView | setPhase の不変条件（phase == .monitoring で ON） | ライフサイクル |
| 8.4 | 監視中の暗転で抑止が継続（解除操作と無関係）／非監視では OS 標準 | PostureSessionManager | enterDimMode/exitDimMode は wake lock に触れない | ライフサイクル |
