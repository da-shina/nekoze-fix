...
final class DistanceMetricIntegrationTests: XCTestCase {
    ...
    /// ロック側距離が基準比 8% 超へ成長 → dwell 通過後に .slouch → 回復で .good
    ...
    func testAngleOnlyRegression_stillJudges() {
        ...
        XCTAssertEqual(sut.snapshot.displayedPosture, .slouch, "角度OVERで .slouch")
    }
    ...
    func testDistanceThresholdChange_affectsVerdictImmediately() {
        ...
        // テストではフレーム間の実時間がほぼ0のため、dwell gate を即時発火に設定
        manager.setPostureDisplayGateDuration(0)

        let shrunk = frame(rightEarDistance: 0.19) // 基準比 105.6%

        // デフォルト8%（>=108%でOVER）では good（即時反映）
        manager.processDetection(.pose(shrunk))
        XCTAssertEqual(manager.snapshot.displayedPosture, .good)

        // 同一 store の閾値を5%（>=105%でOVER）へ変更 → 即 slouch
