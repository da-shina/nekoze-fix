...
# We'll use fixed ones that we hope are not used. We can generate them from the file content to avoid collisions.
# But for simplicity, let's use the ones we tried before and check if they are used.

# We'll read the backup file and check if the IDs we want to use are already present.
# If they are, we'll generate new ones.

import re

def get_expected_ids(content):
...
existing_ids = get_expected_ids(backup_content)
